...
module FF_QE_nucleonScattering
  ...
  ! Added because of the presence of NSI
  logical,save :: justSMpNSIee=.false.
  logical,save :: justNSIetau=.false.

  logical,save :: justNSImue=.false.
  logical,save :: justSMpNSImumu=.false.
  logical,save :: justNSImutau=.false.

  logical,save :: justSMpNSItautau=.false.
  ...
  real, save :: epVuetau=0
  real, save :: epVdetau=0
  real, save :: epVsetau=0
  real, save :: epAuetau=0
  real, save :: epAdetau=0
  real, save :: epAsetau=0

  real, save :: epVumumu=0
  real, save :: epVdmumu=0
  real, save :: epVsmumu=0
  real, save :: epAumumu=0
  real, save :: epAdmumu=0
  real, save :: epAsmumu=0

  real, save :: epVumutau=0
  real, save :: epVdmutau=0
  real, save :: epVsmutau=0
  real, save :: epAumutau=0
  real, save :: epAdmutau=0
  real, save :: epAsmutau=0

  real, save :: epVutautau=0
  real, save :: epVdtautau=0
  real, save :: epVstautau=0
  real, save :: epAutautau=0
  real, save :: epAdtautau=0
  real, save :: epAstautau=0
...
contains
  ...
  subroutine initInput
    ...
    NAMELIST /ff_QE/ parametrization,MV2,MA_in,&
         useNonStandardMA,deltas,gA8,MA8,axialMonopole,&
         justSMpNSIee,justNSIetau,justNSImue,justSMpNSImumu,justNSImutau,justSMpNSItautau,&
         epVuee,epVdee,epVsee,epAuee,epAdee,epAsee,&
         epVuetau,epVdetau,epVsetau,epAuetau,epAdetau,epAsetau,&
         epVumue,epVdmue,epVsmue,epAumue,epAdmue,epAsmue,&
         epVumumu,epVdmumu,epVsmumu,epAumumu,epAdmumu,epAsmumu,&
         epVumutau,epVdmutau,epVsmutau,epAumutau,epAdmutau,epAsmutau,&
         epVutautau,epVdtautau,epVstautau,epAutautau,epAdtautau,epAstautau
  ...
  subroutine formfactors_QE(QSquared,processID,initialState_charge,F1,F2,FA,FP,GE,GM)
    ...
    case (NC,antiNC)
       ...
       if (initialState_charge.eq.proton) then
          ...
          if (justSMpNSIee) F1=(0.5-2.*sinsthweinbg+2.*epVuee+epVdee)*F1p+(-0.5+epVuee+2.*epVdee)*F1n + (-0.5+epVuee+epVdee+epVsee)*F1S
          if (justSMpNSIee) F2=(0.5-2.*sinsthweinbg+2.*epVuee+epVdee)*F2p+(-0.5+epVuee+2.*epVdee)*F2n + (-0.5+epVuee+epVdee+epVsee)*F2S
          if (justNSImue) F1=(2.*epVumue+epVdmue)*F1p+(epVumue+2.*epVdmue)*F1n + (epVumue+epVdmue+epVsmue)*F1S
          if (justNSImue) F2=(2.*epVumue+epVdmue)*F2p+(epVumue+2.*epVdmue)*F2n + (epVumue+epVdmue+epVsmue)*F2S
          if (justNSIetau) F1=(2.*epVuetau+epVdetau)*F1p+(epVuetau+2.*epVdetau)*F1n + (epVuetau+epVdetau+epVsetau)*F1S
          if (justNSIetau) F2=(2.*epVuetau+epVdetau)*F2p+(epVuetau+2.*epVdetau)*F2n + (epVuetau+epVdetau+epVsetau)*F2S
          if (justSMpNSImumu) F1=(0.5-2.*sinsthweinbg+2.*epVumumu+epVdmumu)*F1p+(-0.5+epVumumu+2.*epVdmumu)*F1n + (-0.5+epVumumu+epVdmumu+epVsmumu)*F1S
          if (justSMpNSImumu) F2=(0.5-2.*sinsthweinbg+2.*epVumumu+epVdmumu)*F2p+(-0.5+epVumumu+2.*epVdmumu)*F2n + (-0.5+epVumumu+epVdmumu+epVsmumu)*F2S
          if (justNSImutau) F1=(2.*epVumutau+epVdmutau)*F1p+(epVumutau+2.*epVdmutau)*F1n + (epVumutau+epVdmutau+epVsmutau)*F1S
          if (justNSImutau) F2=(2.*epVumutau+epVdmutau)*F2p+(epVumutau+2.*epVdmutau)*F2n + (epVumutau+epVdmutau+epVsmutau)*F2S
          if (justSMpNSItautau) F1=(0.5-2.*sinsthweinbg+2.*epVutautau+epVdtautau)*F1p+(-0.5+epVutautau+2.*epVdtautau)*F1n + (-0.5+epVutautau+epVdtautau+epVstautau)*F1S
          if (justSMpNSItautau) F2=(0.5-2.*sinsthweinbg+2.*epVutautau+epVdtautau)*F2p+(-0.5+epVutautau+2.*epVdtautau)*F2n + (-0.5+epVutautau+epVdtautau+epVstautau)*F2S
       else if (initialState_charge.eq.neutron) then
          tau3=-1.  !neutron
          F1=(0.5-2.*sinsthweinbg)*F1n-0.5*F1p - F1S/2.
          F2=(0.5-2.*sinsthweinbg)*F2n-0.5*F2p - F2S/2.
          if (justSMpNSIee) F1=(0.5-2.*sinsthweinbg+2.*epVuee+epVdee)*F1n+(-0.5+epVuee+2.*epVdee)*F1p + (-0.5+epVuee+epVdee+epVsee)*F1S
          if (justSMpNSIee) F2=(0.5-2.*sinsthweinbg+2.*epVuee+epVdee)*F2n+(-0.5+epVuee+2.*epVdee)*F2p + (-0.5+epVuee+epVdee+epVsee)*F2S
          if (justNSImue) F1=(2.*epVumue+epVdmue)*F1n+(epVumue+2.*epVdmue)*F1p + (epVumue+epVdmue+epVsmue)*F1S
          if (justNSImue) F2=(2.*epVumue+epVdmue)*F2n+(epVumue+2.*epVdmue)*F2p + (epVumue+epVdmue+epVsmue)*F2S
          if (justNSIetau) F1=(2.*epVuetau+epVdetau)*F1n+(epVuetau+2.*epVdetau)*F1p + (epVuetau+epVdetau+epVsetau)*F1S
          if (justNSIetau) F2=(2.*epVuetau+epVdetau)*F2n+(epVuetau+2.*epVdetau)*F2p + (epVuetau+epVdetau+epVsetau)*F2S
          if (justSMpNSImumu) F1=(0.5-2.*sinsthweinbg+2.*epVumumu+epVdmumu)*F1n+(-0.5+epVumumu+2.*epVdmumu)*F1p + (-0.5+epVumumu+epVdmumu+epVsmumu)*F1S
          if (justSMpNSImumu) F2=(0.5-2.*sinsthweinbg+2.*epVumumu+epVdmumu)*F2n+(-0.5+epVumumu+2.*epVdmumu)*F2p + (-0.5+epVumumu+epVdmumu+epVsmumu)*F2S
          if (justNSImutau) F1=(2.*epVumutau+epVdmutau)*F1n+(epVumutau+2.*epVdmutau)*F1p + (epVumutau+epVdmutau+epVsmutau)*F1S
          if (justNSImutau) F2=(2.*epVumutau+epVdmutau)*F2n+(epVumutau+2.*epVdmutau)*F2p + (epVumutau+epVdmutau+epVsmutau)*F2S
          if (justSMpNSItautau) F1=(0.5-2.*sinsthweinbg+2.*epVutautau+epVdtautau)*F1n+(-0.5+epVutautau+2.*epVdtautau)*F1p + (-0.5+epVutautau+epVdtautau+epVstautau)*F1S
          if (justSMpNSItautau) F2=(0.5-2.*sinsthweinbg+2.*epVutautau+epVdtautau)*F2n+(-0.5+epVutautau+2.*epVdtautau)*F2p + (-0.5+epVutautau+epVdtautau+epVstautau)*F2S
       else
          write(*,*) 'Error in formfactors_QE! Strange initialState_charge',initialState_charge
       end if
       FA = gA*tau3/(2.*(1.+QSquared/MA**2.)**2.)+FAS/2.
       if (justSMpNSIee) FA = (gA+epAuee*(-gA)-epAdee*(-gA))*tau3/(2.*(1.+QSquared/MA**2.)**2.)+3/2*(epAuee+epAdee)*FA8+(0.5+epAuee+epAdee+epAsee)*FAS
       if (justNSImue) FA = (epAumue-epAdmue)*(-gA)*tau3/(2.*(1.+QSquared/MA**2.)**2.)+3/2*(epAumue+epAdmue)*FA8+(epAumue+epAdmue+epAsmue)*FAS
       if (justNSIetau) FA = (epAuetau-epAdetau)*(-gA)*tau3/(2.*(1.+QSquared/MA**2.)**2.)+3/2*(epAuetau+epAdetau)*FA8+(epAuetau+epAdetau+epAsetau)*FAS
       if (justSMpNSImumu) FA = (gA+epAumumu*(-gA)-epAdmumu*(-gA))*tau3/(2.*(1.+QSquared/MA**2.)**2.)+3/2*(epAumumu+epAdmumu)*FA8+(0.5+epAumumu+epAdmumu+epAsmumu)*FAS
       if (justNSImutau) FA = (epAumutau-epAdmutau)*(-gA)*tau3/(2.*(1.+QSquared/MA**2.)**2.)+3/2*(epAumutau+epAdmutau)*FA8+(epAumutau+epAdmutau+epAsmutau)*FAS
       if (justSMpNSItautau) FA = (gA+epAutautau*(-gA)-epAdtautau*(-gA))*tau3/(2.*(1.+QSquared/MA**2.)**2.)+3/2*(epAutautau+epAdtautau)*FA8+(0.5+epAutautau+epAdtautau+epAstautau)*FAS
       ! We don't take into account NSI in axialMonopole yet, so please don't turn it on when work within NSI
       if (axialMonopole)  &
